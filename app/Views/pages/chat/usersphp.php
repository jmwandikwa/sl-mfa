...
$sql = "SELECT * FROM users WHERE NOT user_id = {$outgoing_id} ORDER BY user_id ASC";

// $sql = "select * FROM users JOIN messages WHERE messages.incoming_msg_id = users.user_id OR messages.outgoing_msg_id = users.user_id WHERE NOT users.user_id = {$outgoing_id} ORDER BY users.user_id ASC";
		...
		$output .= '<div class="chat-box-one-side3" onclick="getChat('.$row['user_id'].')">
				...
				<a a href="#">
					<div class="w-p100 d-flex align-items-center m-1 p-2">
						<img src="/profile/'.$row["user_image"].'" class="bg-light ms-0 me-2 rounded-circle" height="48" alt="Brandon Smith" />
